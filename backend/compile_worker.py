...
# Read source files
if not os.path.exists(INDEX_HTML) or not os.path.exists(STYLE_CSS) or not os.path.exists(APP_JS):
    print("ERROR: Source files index.html, style.css, or app.js are missing.")
    sys.exit(1)
...
# Escape backticks and placeholder indicators in content since they will be wrapped in template literals
def escape_template_literal(text):
    return text.replace("`", "\\`").replace("${", "\\${")

html_escaped = escape_template_literal(html_content)
css_escaped = escape_template_literal(css_content)
js_escaped = escape_template_literal(js_content)

# Worker template
worker_template = f"""const corsHeaders = {{
...
const cssContent = `{css_escaped}`;
const jsContent = `{js_escaped}`;
...
print("worker.js successfully compiled with static assets!")
